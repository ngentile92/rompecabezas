...
var instrucciones = ["Mira la imagen que debes lograr", "Analizar las posibilidades de movimientos para llegar a dicho objetivo", "Utiliza las flechas de tu teclado para lograrlo!"];
...
function incorporarMovimientoEnArray(direccion) {
...
function chequearSiGano() {
    for (let i = 0; i < grilla.length; i++) {
      for (let j = 0; j < grilla[i].length; j++) {
        var elemento = grilla[i][j];
        if(elemento != grillaGanadora[i][j]){
          return false;
        } 
      }
    }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  ...
  // console.log(grilla[filaPos1][columnaPos1]);
  //console.log(grilla[filaPos2][columnaPos2]);
}
...
function moverEnDireccion(direccion) {
  ...
  //COMPLETAR: Agregar la dirección del movimiento al arreglo de movimientos
        incorporarMovimientoEnArray(direccion);
